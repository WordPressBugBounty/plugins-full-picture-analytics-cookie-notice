...
switch( $section_id ){
	...
	case 'fupi_clar_install':
		...
		$clar_id = ! empty ( $this->settings ) && ! empty ( $this->settings['id'] ) ? esc_attr( $this->settings['id'] ) : '';

		$ret_text = '
		<div id="fupi_not_installed_info" class="fupi_installation_status fupi_hidden">
			<img src="' . FUPI_URL . 'admin/assets/img/almost_ico.png" aria-hidden="true"> <p>' . sprintf( esc_html__( '%1$s is not installed', 'full-picture-analytics-cookie-notice' ), 'Microsoft Clarity' ) . '<br><span class="fupi_small">' . esc_html__( 'To install it, please fill in the required field below', 'full-picture-analytics-cookie-notice' ) . '</span>.</p>
		</div>
		<div id="fupi_installed_info" class="fupi_installation_status fupi_hidden">
			<img src="' . FUPI_URL . 'admin/assets/img/success_ico.png" aria-hidden="true"> <p>' . esc_html__( 'Well done! Microsoft Clarity is installed', 'full-picture-analytics-cookie-notice' ) . '<br><span class="fupi_small">' . esc_html__( 'The data is sent to a project with ID ', 'full-picture-analytics-cookie-notice' ) . $clar_id . '</span>.</p>
		</div>';
	...
	case 'fupi_clar_loading':
		$ret_text = '<p>' . esc_html__( 'If you have consent banner enabled in the opt-in or one of automatic modes, Clarity will start working after visitors consent to using their data for statistics. This can be avoided if you enable the consent mode below.', 'full-picture-analytics-cookie-notice') . '</p>';
	break;

	// TAGS

	case 'fupi_clar_tags':
		$ret_text = '<p>' . esc_html__( 'Use these settings to tag session recordings with actions and data they registered.', 'full-picture-analytics-cookie-notice' ) . '</p>';
	break;
