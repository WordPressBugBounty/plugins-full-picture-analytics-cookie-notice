...
switch( $section_id ){
	...
	case 'fupi_twit_loading':
		$ret_text = '<p>' . esc_html__( 'If you have consent banner enabled in the opt-in or one of automatic modes, X / Twitter Ads will start working after visitors consent to using their data for statistics and marketing purposes.', 'full-picture-analytics-cookie-notice') . '</p>';
	break;

	case 'fupi_twit_single':

		$ret_text = '<p>' . sprintf( esc_html__( 'Here you can set up tracking events that happen on your site. Learn %1$show to fill in the fields below%2$s.', 'full-picture-analytics-cookie-notice'), '<button type="button" class="fupi_open_popup fupi_faux_link" data-popup="fupi_track_actions_popup">', '</button>' ) . '</p>';
