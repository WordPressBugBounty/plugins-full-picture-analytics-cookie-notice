	...
	case 'fupi_fbp1_ecomm':
		...
		if ( empty( $this->enable_woo_descr_text ) ) {
			...
			return '<p>' . esc_html__( 'You are now automatically tracking WooCommerce events:', 'full-picture-analytics-cookie-notice') . '</p>
				...
				<p>' . esc_html__( 'If you enable the Conversion API, then all these events will be also automatically sent via server-side tracking technology.', 'full-picture-analytics-cookie-notice') . '</p>
