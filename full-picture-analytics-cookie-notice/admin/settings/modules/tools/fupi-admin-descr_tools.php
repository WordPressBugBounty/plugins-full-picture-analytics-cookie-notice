...
switch( $a['id'] ){
	...
	case 'fupi_tools_integrations':
		...
		return '<div class="fupi_descr_buttons_wrappers">
	...
	case 'fupi_tools_tagmanagers':
		return '<p>' . esc_html__( 'Install tools not listed above.', 'full-picture-analytics-cookie-notice') . '</p>';
	break;

	// PRIVACY

	case 'fupi_tools_privacy':
		return '<p>' . esc_html__( 'Track visitors in compliance with GDPR, PiPEDA, CCPA and other privacy laws.', 'full-picture-analytics-cookie-notice') . '</p>';
	break;

	// EXTENSIONS

	case 'fupi_tools_ext':
		return '<p>' . esc_html__( 'Add extra features to other modules.', 'full-picture-analytics-cookie-notice') . '</p>';
	break;
