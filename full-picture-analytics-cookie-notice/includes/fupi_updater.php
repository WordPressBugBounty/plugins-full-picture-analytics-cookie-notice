...
if ( fupi_fs()->is_not_paying() ) {
    // UPDATE TO V6.0 and send email about changes in the FREE version
    if ( version_compare( $prev_version, '6.0.0' ) == -1 ) {
        // 0 if equal, -1 if prev is lower
        $updated = true;
        // prepare and send email to admin
        $recipient = get_bloginfo( 'admin_email' );
        $email_headers = 'Content-Type: text/html; charset=UTF-8';
        $email_title = 'Important changes to the plugin WP Full Picture Free for ' . get_bloginfo( 'url' );
        $email_body = 'This is an automatic message sent by the plugin from your website.<br><br>
    
        Dear admin of ' . get_bloginfo( 'url' ) . ' and a user of WP Full Picture Free plugin.<br><br>
        The new version of WP Full Picture Free comes with a major change in available features. It gives you many powerful tracking features, but also takes away some.<br><br>
        If you have not updated your plugin for more then 2 months, then it is possible, you have not received our previous automatic messages informing about it, though.<br><br>
        You can learn about the changes on this page >> <a href="https://wpfullpicture.com/blog/changes-to-the-free-version-of-wp-full-picture/">click here</a>.<br><br>
        If you are not happy with these changes, you can revert back to the <a href="https://downloads.wordpress.org/plugin/full-picture-analytics-cookie-notice.5.3.0.zip">previous version</a>.<br><br>
...
if ( $updated ) {
    ...
    $versions[1] = $this->version;
    update_option( 'fupi_versions', $versions );
    include FUPI_PATH . '/admin/settings/fupi-clear-cache.php';
}