...
$current_texts = [
    ...
    'notif_descr'       => ( !empty( $notice_opts['notif_text'] ) ? fupi_modify_cons_banner_text( esc_html( $notice_opts['notif_text'] ) ) : fupi_modify_cons_banner_text( $default_texts['notif_descr'] ) ),
    'necess_descr'      => ( !empty( $notice_opts['necess_text'] ) ? fupi_modify_cons_banner_text( esc_html( $notice_opts['necess_text'] ) ) : '' ),
    'stats_descr'       => ( !empty( $notice_opts['stats_text'] ) ? fupi_modify_cons_banner_text( esc_html( $notice_opts['stats_text'] ) ) : $default_texts['stats_descr'] ),
    'pers_descr'        => ( !empty( $notice_opts['pers_text'] ) ? fupi_modify_cons_banner_text( esc_html( $notice_opts['pers_text'] ) ) : $default_texts['pers_descr'] ),
    'market_descr'      => ( !empty( $notice_opts['marketing_text'] ) ? fupi_modify_cons_banner_text( esc_html( $notice_opts['marketing_text'] ) ) : $default_texts['market_descr'] ),
];
// CLASSES & DATA ATTR
...
if ( isset( $popup_max_width_val ) ) {
    $popup_max_width = ( empty( $popup_max_width_val ) ? '520px' : esc_attr( $popup_max_width_val ) . 'px' );
} else {
...
if ( $this->is_customizer || (!$hidden_elements || $hidden_elements && !in_array( 'settings_btn', $hidden_elements )) ) {
...
if ( isset( $this->tools['geo'] ) ) {
    if ( isset( $this->cook['mode'] ) && $this->cook['mode'] === 'notify' ) {
        $is_mode_notify = true;
    }
} else {
    if ( isset( $this->cook['enable_scripts_after'] ) && $this->cook['enable_scripts_after'] === 'notify' ) {
        $is_mode_notify = true;
    }
}
if ( !$is_mode_notify && ($this->is_customizer || isset( $notice_opts ) && !empty( $notice_opts['enable_toggle_btn'] )) ) {
    $image_file_id = get_theme_mod( 'fupi_custom_toggler_img' );
    $image_file_src = wp_get_attachment_image_url( $image_file_id );
    $img_src = ( !empty( $image_file_src ) ? $image_file_src : FUPI_URL . 'public/cookie-notice/img/fupi_cookie_ico.png' );
