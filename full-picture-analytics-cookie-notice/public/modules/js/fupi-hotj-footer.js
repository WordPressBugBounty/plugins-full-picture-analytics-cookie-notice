FP.fns.hotj_woo_events = () => {
	...
	function track_items( data, event_name, action ){
		...
		let to_track = fp.hotj[ 'tag_woo_' + action + '_data' ];
		
		if( to_track && to_track.length > 0 ) data.products.forEach( prod_a => {

			let prod = prod_a[0];

			if ( to_track.includes('p_id') ) hj( 'event', event_name + ' prod id: ' + FP.fns.get_woo_prod_id( prod ) );
			if ( to_track.includes('p_name') ) hj( 'event', event_name + ' prod: ' + FP.fns.get_woo_prod_name( prod ) );
		} );
	...
	function track_cart( type ){ // type can be either "checkout" or "order"
		...
		let to_track = type == 'checkout' ? fp.hotj[ 'tag_woo_checkouts_data' ] : fp.hotj[ 'tag_woo_purchases_data' ];
		if ( ! to_track ) return;
		...
		if ( type == 'order' && to_track.includes('id') ) {
