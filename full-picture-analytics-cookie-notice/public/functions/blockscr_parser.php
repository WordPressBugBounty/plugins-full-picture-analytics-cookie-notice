...
if ( ! empty( $this->blockscr['auto_rules'] ) ) {
    foreach ( $this->blockscr['auto_rules'] as $source ) {
        switch ( $source ) {
...
if ( ! empty( $this->blockscr['blocked_scripts'] ) ) {
    $blocking_rules = array_merge( $blocking_rules, $this->blockscr['blocked_scripts'] );
}

// Go over all the rules and block what needs to be blocked

foreach ( $blocking_rules as $scr_data ) {
