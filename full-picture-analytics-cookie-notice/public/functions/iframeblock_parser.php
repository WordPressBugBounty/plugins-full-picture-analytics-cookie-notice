...
if ( ! empty( $iframeblock['auto_rules'] ) ) {

    foreach ( $iframeblock['auto_rules'] as $source ) {
        switch ( $source ) {
            case 'youtube':
                $blocking_rules[] = array(
                    'name'          => 'youtube.com',
...
if ( ! empty( $iframeblock['manual_rules'] ) ) {
    $blocking_rules = array_merge( $blocking_rules, $iframeblock['manual_rules'] );
}
...
$new_html = preg_replace_callback( '/<body([^>]*)>(.*?)<\/body>/is', function( $body_matches ) use ( $blocking_rules ){
    ...
    $bodyContent_2 = preg_replace_callback( '/<iframe([^>]*)>(.*?)<\/iframe>/is', function( $iframe ) use ( $blocking_rules ) {
        
        $ret_val = "<iframe{$iframe[1]}>{$iframe[2]}</iframe>";

        if ( count( $blocking_rules ) > 0 ) {

            foreach ( $blocking_rules as $iframe_settings ) {
                ...
                if ( preg_match( $pattern, $iframe[1] ) ) {
                    ...
                    $ret_val = '<div class="fupi_blocked_iframe" data-stats="' . $stats . '" data-market="' . $market . '" data-pers="' . $pers . '" ' . $placeholder . $privacy . $name . '><div class="fupi_iframe_data" ' . $iframe[1] . '>' . $iframe[2] . '</div></div><!--noptimize--><script data-no-optimize="1">FP.manageIframes();</script><!--/noptimize-->';
